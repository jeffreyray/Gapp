...
sub build_widget {
	...
	if ( $scrollbars ) {
		...
		# if the widget is already parented by a scrolled window, we
	        # don't need to do it, just set the policy of the scrollbars
		if ($widget->get_gtk_parent_widget->isa('Gtk2::ScrolledWindow')) {
		    $widget->get_gtk_parent_widget->set_policy(@{$scrollbars});
		}
		
		# otherwise decide how to implement scrollbars
		else {
		    my $sw = Gtk2::ScrolledWindow->new;
		    
		    # use scrolled window without viewport of widget supports it
		    my $gtk_parent_widget = $widget->get_gtk_parent_widget;
		    if ($gtk_parent_widget->isa('Gtk2::TextView') ||
			    $gtk_parent_widget->isa('Gtk2::IconView') ||
			    $gtk_parent_widget->isa('Gtk2::TreeView')) {
			
			    $sw->add($widget->get_gtk_parent_widget);
		    }
		    # otherwise added widget to scrolled window with viewport
		    else {
...
sub build_calendar {
	...
	my $gtk_entry = Gtk2::Calendar->new;
...
sub build_spin_button {
	my $self = shift;
	my ($entry) = @_;
	
	my $gtk_entry = Gtk2::SpinButton->new_with_range( @{$entry->get_range} );
	$gtk_entry->set_digits( $entry->get_digits );
